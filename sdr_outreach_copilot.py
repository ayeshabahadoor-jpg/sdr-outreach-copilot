...
class OutreachSequence:
    ...
    def to_markdown(self, prospect: Prospect,
        ...
        """
        if score is None:
            score = score_personalization(prospect, self)
        n = score["touches"]
        score_cell = (
            f"{score['score']}/100 ({score['rating']}) — "
            f"intent {score['intent']}/{n}, pain {score['pain']}/{n}, "
            f"proof {score['proof']}/{n}"
        )
        lines = [
            ...
            f"| Personalization score | {score_cell} |",
...
#: Distinctive fragments that indicate a real Intercom/Fin proof point is
#: present in a piece of copy (numbers/brands, robust to paraphrasing).
PROOF_MARKERS = ("59%", "67%", "12,000", "31%", "lightspeed",
                 "calendly", "chili piper")

#: Common words ignored when matching a phrase against copy.
_STOPWORDS = {
    "the", "a", "an", "and", "or", "to", "of", "in", "on", "at", "for",
    "your", "you", "our", "with", "without", "is", "are", "that", "this",
    "as", "by", "from", "into", "more", "same", "team", "teams",
}

#: How the total 100 points are split across the three dimensions.
SCORE_WEIGHTS = {"intent": 35, "pain": 35, "proof": 30}


def _phrase_hits(text: str, phrase: str, threshold: float = 0.6) -> bool:
    """Return True if enough of ``phrase``'s content words appear in ``text``.

    Token-overlap match (case-insensitive, stopwords removed) so paraphrased
    AI copy still counts, not just verbatim offline copy.
    """
    text_lower = text.lower()
    tokens = [
        tok for tok in re.findall(r"[a-z0-9%]+", phrase.lower())
...
def _has_proof(text: str, prospect: "Prospect") -> bool:
    """Return True if the copy references a real proof point."""
    lower = text.lower()
    if any(marker in lower for marker in PROOF_MARKERS):
        return True
    # Fallback for paraphrased proof points that dropped the number.
    return _phrase_hits(text, prospect.proof_point, threshold=0.5)


def score_personalization(prospect: "Prospect",
                          sequence: "OutreachSequence") -> Dict[str, Any]:
    """Score how well a sequence personalizes to the prospect (0-100).

    Rewards each of the four touches for referencing the intent signal, the
    role-specific pain, and a proof point. Returns the total plus a per-
    dimension breakdown (how many of the 4 touches hit each dimension) so weak
    drafts are easy to spot at a glance.
    """
    touches = [
        " ".join(sequence.email_subjects) + " " + sequence.email_body,
        sequence.linkedin_note,
        sequence.follow_up,
        sequence.call_opener,
    ]
    n = len(touches)

    intent_hits = sum(_phrase_hits(t, prospect.intent_signal) for t in touches)
    pain_hits = sum(_phrase_hits(t, prospect.pain) for t in touches)
    proof_hits = sum(_has_proof(t, prospect) for t in touches)

    total = round(
        SCORE_WEIGHTS["intent"] * intent_hits / n
        + SCORE_WEIGHTS["pain"] * pain_hits / n
        + SCORE_WEIGHTS["proof"] * proof_hits / n
    )
...
def save_markdown(prospect: Prospect, sequence: OutreachSequence,
                  output_dir: Path, score: Optional[Dict[str, Any]] = None) -> Path:
    """Write the sequence to ``output_dir`` and return the path written."""
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / safe_filename(prospect)
    path.write_text(sequence.to_markdown(prospect, score), encoding="utf-8")
    return path


def print_sequence(prospect: Prospect, sequence: OutreachSequence,
    ...
    print(f"  Personalization score: {score['score']}/100 ({score['rating']})"
          f"  [intent {score['intent']}/{n}, pain {score['pain']}/{n}, "
          f"proof {score['proof']}/{n}]")
    print(rule)
...
def print_summary(rows: List[Dict[str, str]]) -> None:
    ...
    if not rows:
        return
    headers = ("Prospect", "Company", "Score", "File saved")
    keys = ("prospect", "company", "score", "file")
    ...
    for row in rows:
        print(fmt(tuple(row[key] for key in keys)))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for prospect in prospects:
        ...
        score = score_personalization(prospect, sequence)
        print_sequence(prospect, sequence, score)
        saved = save_markdown(prospect, sequence, args.output_dir, score)
        summary_rows.append({
            "prospect": prospect.name,
            "company": prospect.company,
            "score": f"{score['score']}/100 {score['rating']}",
            "file": str(saved),
        })
